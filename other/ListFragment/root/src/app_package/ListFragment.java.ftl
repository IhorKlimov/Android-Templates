...
import ${applicationPackage}.databinding.${fragment_layout_list?replace('_', ' ')?capitalize?replace(' ','')}Binding;
</#if>
...
<#if includeFactory>
    ...
    public View onCreateView(LayoutInflater inflater, ViewGroup container,
 	...
 	${fragment_layout_list?replace('_', ' ')?capitalize?replace(' ','')}Binding binding = DataBindingUtil.inflate(LayoutInflater.from(getContext()), R.layout.${fragment_layout_list}, container, false);
	...
	if (mColumnCount <= 1) {
        ...
        binding.list.setAdapter(new ${adapterClassName}(DummyContent.ITEMS, mListener));
